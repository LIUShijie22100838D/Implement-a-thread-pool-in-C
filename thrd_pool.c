...
static task_queue_t *
__taskqueue_create(){
    int ret;  //malloc will return a void*, so a type casting should be applied.
...
__add_task(task_queue_t *queue, void *task){
    ...
    void **link = (void**)task;
    *link = NULL; //task->next = NULL; the link attribute is not limited to "next"
    
    spinlock_lock(&queue->lock);
    *queue->tail = link; // equal to queue->tail->next = link
    queue->tail = link;
...
__pop_task(task_queue_t *queue){
    ...
    void **link = (void**) task; // link is the first 8 byte of the task, can be treated as *next
    queue->head = *link;
...
static inline void * // the interface for the consumer 
__get_task(task_queue_t *queue){
    task_t *task;
    //while loop can check whether the thread is waked up if 
    while((task = __pop_task(queue))==NULL){
        ...
        //2.The current thread is sleep in cond
        //until pthread_cond_signal (notify one) or pthread_cond_broadcast (notify all) is executed 
        //3. waik up the current thread;
...
// static void 
// __thread_terminate(thrdpool_t *pool){
//     atomic_store(&pool->quit,1);
//     __nonblock(pool->task_queue);
//     int i;
//     for (i=0;i<pool->thrd_count;i++){
...
//         pthread_join(pool->threads[i], NULL);
//     }
//     printf("   __taskqueue_destroy(pool->task_queue);\n");
//     __taskqueue_destroy(pool->task_queue);
//     free(pool->threads);
//     free(pool);
//     printf("done 2 \n");
